...
{
    abstract class DmSqlExecuter<T> : SqlExecuter<T> where T : new()
    {
        protected DmSqlExecuter(int? commandTimeout, string owner) : base(commandTimeout, owner)
        {
        }
        ...
        /// <summary>
        /// When overriding in Dm, use base.AddParameters(command) or EnsureDmBindByName(command)
        /// </summary>
        /// <param name="command">The command.</param>
        protected override void AddParameters(DbCommand command)
        {
            EnsureDmBindByName(command);
        }

        protected void EnsureDmBindByName(DbCommand command)
        {
            //Dm.DataAccess.Client only binds first parameter match unless BindByName=true
